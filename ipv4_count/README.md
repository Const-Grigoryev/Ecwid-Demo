...
7. Строго говоря, перфекционистская цель в $O(1)$ аллокаций достигнута не была: один временный объект создаётся каждый
...
несмотря на подробные комментарии, и это может затруднить его поддержку. Зато с точки зрения быстродействия парсер
безупречен: в основном цикле моего кода нет ни единого `new`!

К сожалению, того же нельзя сказать про библиотечный код. Декодер Юникода, как и других стандартных кодировок, показал
себя крайне медленным по сравнению с остальным кодом — главным образом, за счёт копирования между разнотипными буферами.
Решение этой проблемы запланировано на ближайший релиз.

#### Дизайн парсера: интерфейс

В настоящий момент, парсер выполнен в виде отдельного класса `IPv4Parser` с единственным важным методом `parseNextLine`.
С таким подходом имеется ряд концептуальных проблем, главная из которых: непонятно, *что такое* этот ваш парсер.
Что означает изменения состояния объекта `IPv4Parser`?

Концептуальное непонимание выражается в нестройном, нелогичном интерфейсе. В частности, имеющаяся сигнатура
`parseNextLine` не отражает тот факт, что результат парсинга на самом деле является типом-суммой:
```haskell
data ParseResult = Address Int | Error String | BlankLine | EndOfFile
```
Объединив `LineToken` и `IPv4Builder` в один *visitor*, можно будет исправить этот недочёт, а заодно и избавиться
от поля `lastError`, которое тоже выглядит как-то... стрёмно[^get-last-error]. (Дороже ли визитор по сравнению
с одиночной лямбдой? Вроде бы нет.)

[^get-last-error]: Каждый раз, когда вижу `getLastError()`, меня накрывают вьетнамские флешбэки про WinAPI.
В XXI веке так писать, конечно, уже нельзя.

Или можно поступить иначе: пусть `parseNextLine` возвращает `long`. В такое расширение влезает и код результата
(т.е. `ParseResult`), и прочитанное значение адреса. Также можно получить небольшое ускорение, избавившись от вызова
колбэка. С другой стороны, придётся отказаться от гарантий, предоставляемых системой типов, и пожертвовать читаемостью
кода.

Есть и другая претензия. Почему `Reader` прибит к парсеру гвоздями? Это плохо с точки зрения переиспользования кода.
Например, легко вообразить ситуацию, когда файл состоит из разнородных данных, и IP-адрес — только один из возможных
вариантов. Тогда все парсеры для разных типов данных будут владеть одним входным потоком? Звучит несколько бредово.

Переосмысление роли парсера — ещё одна задача на ближайший релиз.
...
- [ ] Кодировку по умолчанию сменить на более легковесную однобайтовую самоделку, в которой были бы только цифры, точки
  и переводы строк. Потенциально, это может очень сильно забустить производительность.
...
- [ ] Редизайн интерфейса `IPv4Parser`. Претензии к существующему дизайну и варианты их разрешения обсуждаются
  в [соответствующем разделе](#дизайн-парсера-интерфейс).
